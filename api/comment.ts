...
것들이다. 자료에 그 항목이 적혀 있지 않으면 절대 쓰지 마라. 짐작·추측·미화 모두 금지다.
 · 수업 태도·집중·질문·숙제 (자료에 **'오늘 수업 태도'** 줄이 없으면.
   그 줄이 있으면 선생님이 직접 체크한 사실이니 **거기 적힌 것만** 쓸 수 있다 —
   '집중 흐트러짐'을 '조금 산만했지만 후반엔 나아졌다'처럼 부풀리지 마라)
 · 표정, 자세, 앉아 있던 모습, 친구와 어땠는지
...
const ANGLES: { key: string; text: string }[] = [
  ...
  // 선생님이 '오늘 수업 태도'를 직접 체크한 날만 열린다(자료에 그 줄이 있을 때만).
  { key: 'attitude', text: "자료의 '오늘 수업 태도'에 적힌 것만 가지고 오늘 수업 모습 한 가지. 적힌 것 밖으로 나가지 마라." },
]
