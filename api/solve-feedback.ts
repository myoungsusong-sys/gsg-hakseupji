...
- **빨간펜 표시(marks)**: 풀이 이미지에서 틀린(또는 다시 볼) 부분의 위치를 이미지 기준 0~1 정규화 좌표 사각형으로 최대 3개까지 표시한다.
  x,y=사각형 왼쪽위, w,h=너비·높이. label은 그 자리에 붙일 6자 이내 짧은 말(예: "부호 확인", "여기부터"). 틀린 부분이 없거나 위치를 특정할 수 없으면 빈 배열.
- 반드시 아래 JSON 한 줄로만 답한다(설명·코드블록 없이):
{"hasWork": true/false, "correct": true/false/null, "feedback": "학생에게 보여줄 피드백", "marks": [{"x":0.1,"y":0.2,"w":0.3,"h":0.1,"label":"부호 확인"}]}
...
export default async function handler(req: any, res: any) {
  ...
  try {
    ...
    let out = { hasWork: true as boolean, correct: null as boolean | null, feedback: text, marks: [] as Mark[] }
    const m = text.match(/\{[\s\S]*\}/)
    if (m) {
      try {
        const j = JSON.parse(m[0])
        const clamp = (v: any) => Math.max(0, Math.min(1, Number(v) || 0))
        const marks: Mark[] = Array.isArray(j.marks)
          ? j.marks.slice(0, 3).map((k: any) => ({
              x: clamp(k?.x), y: clamp(k?.y), w: clamp(k?.w), h: clamp(k?.h),
              label: String(k?.label ?? '').slice(0, 12),
            })).filter((k: Mark) => k.w > 0 && k.h > 0)
          : []
        out = { hasWork: j.hasWork !== false, correct: j.correct ?? null, feedback: String(j.feedback ?? text), marks }
      } catch { /* keep raw */ }
    }
