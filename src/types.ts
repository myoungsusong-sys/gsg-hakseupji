...
export interface Worksheet {
  ...
  author: string
  grade: string
  subject?: '수학' | '과학' | '사회' | '역사' | '영어' | '국어'   // 출제 과목 (없으면 레거시 = 수학). 헤더 과목 스위처 목록 필터용
  tags: string[]
  theme: ThemeKey
  problemIds: string[]
...
export interface Workbook {
  id: string
  subject?: '수학' | '과학' | '사회' | '역사' | '영어' | '국어'   // 과목 (없으면 course로 유도, 그래도 없으면 수학)
...
export interface GradeResult {
  ...
  // ── 틀린 문제 다시 풀기(재채점) — 학생 자가채점(교재·학습지) ──
  attempts?: number      // 시도 횟수 (1=첫 채점 · 2=재시도까지). 없으면 1로 간주
  careless?: boolean     // 실수 — 처음엔 틀렸지만 다시 풀어 맞힘(아는데 실수). 통계에서 진짜 오답과 구분
  retryAnswer?: string   // 2차 시도에 낸 답 (표시·기록용)
  // ── 자기채점 (그래프·작도·서술형처럼 기계 대조가 불가능한 문항) ──
  // 학생이 공책에 풀고 정답(앱의 정답 그림 또는 교재 해설)과 맞춰본 뒤 스스로 ○/✕를 표시한 기록.
  // 자동채점과 섞이지 않게 구분해 둔다 — 정답률 통계에서 신뢰도를 달리 볼 수 있어야 하므로.
  self?: boolean
...
export interface SavedReport {
  ...
  subject?: '수학' | '과학' | '사회' | '역사' | '영어' | '국어'   // 과목 (없으면 레거시 = 수학). 같은 날짜라도 과목별로 별개 보고서다
  studentId: string
