...
export interface Worksheet {
  ...
  author: string
  grade: string
  subject?: '수학' | '과학' | '사회' | '역사'   // 출제 과목 (없으면 레거시 = 수학). 헤더 과목 스위처 목록 필터용
  tags: string[]
  theme: ThemeKey
  problemIds: string[]
...
export interface Workbook {
  id: string
  subject?: '수학' | '과학' | '사회' | '역사'   // 과목 (없으면 course로 유도, 그래도 없으면 수학)
...
export interface SavedReport {
  ...
  subject?: '수학' | '과학' | '사회' | '역사'   // 과목 (없으면 레거시 = 수학). 같은 날짜라도 과목별로 별개 보고서다
  studentId: string
