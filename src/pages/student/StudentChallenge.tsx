...
import { CURRICULA, curriculumFor, defaultCurriculumForGrade, typeName } from '../../data/curriculum'
...
// 과정 선택(학생 학년 기본) + 추천유형만 토글 + 단원 검색
// 소단원 아코디언 → 유형 카드 = 개념(하~중하)|기본(중)|심화(상~최상) 3밴드 슬롯 그리드
...
export default function StudentChallenge() {
  ...
  // 과정: 학생 학년 기본, 문제 풀이 있는 과정만 선택 가능
  const courses = useMemo(() => CURRICULA.filter(c => (POOL_COURSES as readonly string[]).includes(c.id)), [])
  const [courseId, setCourseId] = useState(() => {
    const d = defaultCurriculumForGrade(me.grade)
    return (POOL_COURSES as readonly string[]).includes(d) ? d : 'm1-1'
  })
  ...
  const loaded = [...byType.values()].some(arr => arr.length > 0)
